...
namespace SharpMath.Optimization
{
	...
	public sealed class BfgsOptimizer : VariableConstrainedOptimizer
	{
		...
		/// <summary>
		/// The number of corrections in the BFGS scheme. If not specified, the maximally recommended value is used.
		/// </summary>
		public int? BfgsCorrections
		{
			get;
			set;
		}
		...
		private class InnerBfgsOptimizer : InnerVariableConstrainedOptimizer
		{
			...
			public override IOptimizerResult Run(IPoint initialPoint)
			{
				...
				double optimalValue = ObjectiveFunction.Value(CreateEvaluator(Transform(x)));
			...
			*/
			...
			private void funcgrad(ref double[] x,
				...
				ref double[] g)
			{
				IEvaluator evaluator = CreateEvaluator(Transform(x));

				f = function.Value(evaluator);
				for (int i = 0; i < n; i++)
				{
					g[i + 1] = gradient[i].Value(evaluator);
				}
			}
