...
namespace SharpMath.Optimization.Ipopt
{
	...
	public class IpoptOptimizer : ConstrainedOptimizer
	{
		public IpoptOptimizer()
		{
			...
			Options.Add("max_iter", 3000);
			Options.Add("max_cpu_time", 1.0e6);
			Options.Add("print_level", 0);
		}
		...
		public double MaxCpuSeconds
		{
			get
			{
				return (double)Options["max_cpu_time"];
			}
			set
			{
				Options["max_cpu_time"] = value;
			}
		}
		...
		public int PrintLevel
		{
			get
			{
				return (int)Options["print_level"];
			}
			set
			{
		...
		private class InnerIpoptOptimizer : InnerConstrainedOptimizer
		{
			...
			private double[] x_L, x_U, g_L, g_U;
			...
			private Function[] constraintFunctions, objectiveFunctionGradient;
			...
			private List<IpoptOption> options;
			private EventHandler<IpoptIntermediateEventArgs> intermediate;
			private int currentIteration;
			private IEvaluator currentEvaluator;
				...
				: base(optimizer)
			{
				...
				options.Add(new IpoptNumberOption("obj_scaling_factor", ObjectiveFunctionScaling));
			...
			{
				if (currentEvaluator == null || new_x)
				{
					// Reuse same evaluation object until instructed to change point. This makes caching (slightly) more efficient.
					currentEvaluator = CreateEvaluator(x);
				}

				try
				{
					obj_value = objectiveFunction.Value(currentEvaluator);
					Validate(obj_value);
				}
			...
			private bool eval_grad_f(int n, double[] x, bool new_x, double[] grad_f)
			{
				...
				try
				{
					for (int i = 0; i < n; i++)
					{
						grad_f[i] = objectiveFunctionGradient[i].Value(currentEvaluator);
						Validate(grad_f[i]);
					}
				}
				catch (ArithmeticException)
			...
			private bool eval_g(int n, double[] x, bool new_x, int m, double[] g)
			{
				...
				try
				{
					for (int i = 0; i < m; i++)
					{
						g[i] = constraintFunctions[i].Value(currentEvaluator);
						Validate(g[i]);
					}
				}
				catch (ArithmeticException)
				...
				else
				{
					...
					try
					{
						for (int i = 0, l = 0; i < m; i++)
						{
							...
							for (int j = 0; j < gradient.Count; j++, l++)
							{
								values[l] = gradient[j].Function.Value(currentEvaluator);
								Validate(values[l]);
							}
						}
					}
				  ...
				  double[] values)
			{
				...
				else
				{
					...
					try
					{
						...
						{
							double y = entry.Function.Value(currentEvaluator);
							Validate(y);
